# -*- coding: utf-8 -*-
"""eval_dsc.ipynb — SR_Output vs CycleGAN vs CUT, Dual-Track 지표를 직접 실행."""
import json, os
cells = []
...
- 학습된 CUT(`checkpoints/cut_big_dsc`)과 기업 CycleGAN(`best_G_AB.pth`)을 디스크에서 로드 → 재학습 불필요
...
CUT은 `checkpoints/cut_big_dsc`에서, CycleGAN은 `best_G_AB.pth`에서 로드. (SR_Output은 원본 그대로가 조건)""")
code("""# CUT netG: CUT 프레임워크로 체크포인트 로드
sys.argv = ["test.py", "--dataroot", os.path.join(ROOT,"refiners/data/refine_big"),
            "--name", "cut_big_dsc", "--CUT_mode", "CUT", "--phase", "test", "--gpu_ids", "0",
...
outp = os.path.expanduser("~/geoit_sr_refiner/eval_dsc.ipynb")
